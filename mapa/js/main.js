...
function pridejAkvarelVzory(vynutit) {
  ...
  vyrob('vzor-pole', SEZ.poleZaklad, (ctx, torus) => {
    ...
    // ⭐ engine 264 („pole více oraná"): brázda = DVOJICE tmavý hřbet + světlý
    // hřeben posunutý kolmo o 1,4 px, výraznější (0,10 → 0,17) a hustší.
    for (let i = 0; i < kolik(190); i++) {
      const x = r() * S; const y = r() * S;
      const d = (30 + r() * 70) * k;
      const o = 1.4 * k;                          // kolmý posun hřebene
      const w = (0.8 + r() * 1.2) * k;
      torus((ox, oy) => {
        ctx.strokeStyle = 'rgba(90,70,40,0.17)';
        ctx.lineWidth = w;
        ctx.beginPath();
        ctx.moveTo(x + ox, y + oy);
        ctx.lineTo(x + ox + d * 0.85, y + oy - d * 0.5);
        ...
        ctx.lineWidth = w * 0.9;
        ctx.beginPath();
        ctx.moveTo(x + ox + 0.5 * o, y + oy + 0.85 * o);
        ctx.lineTo(x + ox + d * 0.85 + 0.5 * o, y + oy - d * 0.5 + 0.85 * o);
        ctx.stroke();
      });
    }
  ...
  vyrob('vzor-louka', SEZ.loukaZaklad, (ctx, torus) => {
    ...
    // ⭐ engine 264: KVÍTKY – drobné barevné tečky v barvách sezóny („louky
    // více květnaté"; SEZONY.kvitky: jaro bílá/růžová, léto bílá/žlutá,
    // podzim krémová/okrová, sníh bílá). Tečky nejsou tvar, přeskládání
    // dlaždic se na nich nepozná.
    for (let i = 0; i < kolik(120); i++) {
      const x = r() * S; const y = r() * S;
      const pr = (1.0 + r() * 1.6) * k;
      ctx.fillStyle = SEZ.kvitky[r() < 0.55 ? 0 : 1];
      torus((ox, oy) => {
        ctx.beginPath();
...
/// ⭐ engine 232: PŘEPOČTY JEN PŘI ZMĚNĚ POHLEDU. `idle` chodí na stojící mapě
/// ~3×/s (animace), takže se domy, okna, mosty i sídla přepočítávaly pořád
/// a čtyři `setFilter` nutily MapLibre k plnému update stylu (~30
/// překreslení/s v klidu, 270 % CPU). Podpis = kamera + verze odkrytí (+ počet
/// prvků v pohledu, aby dojezd dlaždic přepočet vyvolal).
let odkrytiVerze = 0;
let pohledPodpisBudovy = '', pohledPodpisOkna = '', pohledPodpisMosty = '', pohledPodpisSidla = '';
let budovyFiltrKlic = '';
const zabagedFiltrKlice = new Map();
function podpisPohledu() {
  const c = mapa.getCenter();
  return c.lng.toFixed(5) + '|' + c.lat.toFixed(5) + '|' + mapa.getZoom().toFixed(2) + '|'
    + Math.round(mapa.getBearing()) + '|' + Math.round(mapa.getPitch()) + '|' + odkrytiVerze;
}
...
function nasadBudovyHerni() {
  ...
  if (!nasadBudovyHerni._hooked) {
    ...
    mapa.on('moveend', naplanujBudovyHerni);   // engine 232: domy hned po gestu, ne až po idle
    try {
      if (typeof Mlha !== 'undefined' && Mlha.priObjeveni) {
        Mlha.priObjeveni(() => { odkrytiVerze++; naplanujBudovyHerni(); });
      }
    } catch (e) { /* nic */ }
  }
...
function prepoctiSidlaPopisky() {
  ...
  const ppS = podpisPohledu() + '|' + sidla.length;
  if (ppS === pohledPodpisSidla) return;          // engine 232: v klidu nic
  pohledPodpisSidla = ppS;
  try { if (sidla.length) domy = mapa.querySourceFeatures('omt', { sourceLayer: 'building' }); } catch (e) { return; }
...
function vyrazBarvyOken(noc, krok) {
  ...
  const sviti = proSvetlo(OKNA_BARVA.noc);          // světlo z okna se netlumí
  const tma = proSvetlo(ztlumNoci(OKNA_BARVA.zhasle, krok));
  return ['case',
    ['==', ['feature-state', 'sv'], 1], sviti,
    ['==', ['feature-state', 'sv'], 0], tma,
    ['<', ['coalesce', ['get', 'r'], 50], OKNA_SVITI_PRAH], sviti,
    tma];
}
/// Ztlumí barvu 3D domu podle kroku noci (`NOC_DOMY_TLUM` z nočního překryvu).
function domNoci(hex, krok) {
  if (krok == null) krok = typeof krokNoci === 'number' ? krokNoci : 0;
  const a = (NOC_TMA_KRYTI[Math.max(0, Math.min(3, krok))] || 0) * NOC_DOMY_TLUM;
  if (!a) return hex;
  const c = hexNaRgb(hex);
  return '#' + c.map((x, i) => Math.round(x * (1 - a) + NOC_TMA_BARVA[i] * a)
                                   .toString(16).padStart(2, '0')).join('');
}
/// Přebarví 3D domy podle kroku noci (volá `aplikujNoc`).
function prebarviDomyNoci(krok) {
  const ODK = ['boolean', ['feature-state', 'o'], false];
  const PR = 'rgba(0,0,0,0)';
  const H = ['coalesce', ['get', 'render_height'], 6];
  try {
    if (mapa.getLayer('okolnik-budovy-herni-zdi')) {
      mapa.setPaintProperty('okolnik-budovy-herni-zdi', 'fill-extrusion-color',
          ['case', ODK, domNoci('#EAD9B6', krok), PR]);
    }
    if (mapa.getLayer('okolnik-budovy-herni-strecha')) {
      mapa.setPaintProperty('okolnik-budovy-herni-strecha', 'fill-extrusion-color',
          ['case', ODK, ['case', ['<=', H, 9.5],
            ['match', ['%', ['id'], 3], 0, domNoci('#B9684A', krok), 1, domNoci('#AE6045', krok), domNoci('#C0745A', krok)],
            domNoci('#8E8478', krok)], PR]);
    }
  } catch (e) { /* styl se zrovna mění */ }
}
...
function prepoctiOkna3d() {
  ...
  try {
    window.__casy = window.__casy || {};
    window.__casy.oknaN = features.length; window.__casy.oknaBudov = budov; window.__casy.oknaSpocitano = spocitano;
...
/// `_getTerrainCoordsForRegularTile` počítá pro každou drapovanou dlaždici
/// každý snímek matice ke všem terénním dlaždicím (nové Float32Array +
/// clone) – v profilu gest 4 s ze 42 s. Výsledek závisí jen na klíči
/// dlaždice a množině vykreslitelných terénních dlaždic, ne na kameře →
/// keš (hit 99 %). Snímky nad 33 ms to neměnilo (GPU), šetří CPU a GC.
let terenKesNasazena = false;
function nasadTerenKes() {
  if (terenKesNasazena || !mapa || !mapa.terrain || !mapa.terrain.tileManager) return;
  terenKesNasazena = true;
  try {
    const TM = Object.getPrototypeOf(mapa.terrain.tileManager);
    if (typeof TM._getTerrainCoordsForRegularTile !== 'function' || TM._getTerrainCoordsForRegularTile.__kes) return;
    const orig = TM._getTerrainCoordsForRegularTile;
    const w = function (e) {
      const rk = this._renderableTilesKeys;
      const ver = rk.length + ':' + rk.join(',');
      if (this.__tcVer !== ver) { this.__tcVer = ver; this.__tc = new Map(); }
      const k = e.key;
      let t = this.__tc.get(k);
      if (t) return t;
      t = orig.call(this, e);
      this.__tc.set(k, t);
      return t;
    ...
    w.__kes = orig;
    TM._getTerrainCoordsForRegularTile = w;
...
/// ⛔⛔ SVĚTLO STYLU NÁSOBÍ BARVU KAŽDÉ 3D PLOCHY, a `svetlo.js` ho mění podle
/// slunce a měsíce (ráno #FFB878/0,30, poledne #FFF7E8/0,70, měsíc #A9BDE0,
/// tma #7F8AA3/0,18). Deska zadaná v barvě silnice `#A39D94` (163,157,148) se
/// proto v noci vykreslila jako (82,86,96) – modrá břidlice vedle světle šedé
/// vozovky. Barvy 3D dílů se tedy zadávají CÍLOVĚ (jak mají vypadat) a
/// `proSvetlo()` je před stavbou přepočte podle aktuálního `mapa.getLight()`.
/// ⚠️ Ve slabém světle (noc) na jas vozovky nedosáhnou – zůstane hue, jas
/// klesne (víc `fill-extrusion` v tom světle neumí).
const MOST_CIL = { zel: '#7A6E63', pruh: '#F3EFE4', kolej: '#C8BFB2',
                   nosnik: '#6E675C', pilir: '#9A9287', opera: '#8E877C',
                   zabradli: '#CFC8BA' };
const MOST_CIL_ASFALT = { motorway: '#87827C', trunk: '#87827C', primary: '#928C84',
                          secondary: '#9B958C', tertiary: '#A39D94', minor: '#A39D94',
                          service: '#A39D94', track: '#9A9184', path: '#9A9184' };
const MOST_ASFALT_ZAL = '#A39D94';
/// Ploché díly (obyčejná výplň) – světlo je NENÁSOBÍ, barvy skutečné.
const MOST_BARVY = { deska: '#C9C2B4', hrana: '#6E675C', stin: '#2B2A28' };
...
function hexNaRgb(h) {
  ...
  const m = h.match(/[\d.]+/g) || [255, 255, 255];
  return [+m[0], +m[1], +m[2]];
}
/// Kolikrát světlo stylu ztlumí VODOROVNOU 3D plochu (shader `fill-extrusion`:
/// `mix(1−I, max(1,I), dot(normála, pozice světla)) × barva světla`).
/// Konstanta 1,05 je doměřená na telefonu (shader počítá o kousek světleji).
function faktorSvetla() {
  let L = null;
  try { L = mapa && mapa.getLight && mapa.getLight(); } catch (e) { L = null; }
  const c = hexNaRgb((L && L.color) || '#FFFFFF');
  const I = (L && typeof L.intensity === 'number') ? L.intensity : 0.5;
  const poz = (L && L.position) || [1.15, 335, 45];
  const r = poz[0] == null ? 1.15 : poz[0], pol = poz[2] == null ? 45 : poz[2];
  const d = Math.max(0, Math.min(1, r * Math.cos(pol * Math.PI / 180)));
  const dir = (1 - I) + (Math.max(1, I) - (1 - I)) * d;
  return [dir * c[0] / 255 * 1.05, dir * c[1] / 255 * 1.05, dir * c[2] / 255 * 1.05];
}
/// ⛔ Noční ztmavovací překryv (`noc-prekryv`, #081226 s krytím TMA) leží POD
/// 3D vrstvami: silnici ztmaví, desku mostu ne. Cílová barva se proto ztlumí
/// stejně, jinak most v noci SVÍTÍ (změřeno: deska 153,149,141 proti silnici
/// 42,49,62). ⚠️ Ploché díly mostu leží pod překryvem, ty se netlumí.
const NOC_TMA_KRYTI = [0, 0.20, 0.40, 0.66];   // engine 264: světlejší noc (viz TMA)
const NOC_TMA_BARVA = [11, 26, 58];            // engine 264: #0B1A3A
function ztlumNoci(hex, krok) {
  // ⛔ `krokNoci` se v `aplikujNoc` nastavuje AŽ NA KONCI – kdo tlumí během ní,
  // musí krok předat, jinak počítá se starým (engine 247: okna zůstala denní).
  if (krok == null) krok = typeof krokNoci === 'number' ? krokNoci : 0;
  const a = NOC_TMA_KRYTI[Math.max(0, Math.min(3, krok))] || 0;
  if (!a) return hex;
  const c = hexNaRgb(hex);
  return '#' + c.map((x, i) => Math.round(x * (1 - a) + NOC_TMA_BARVA[i] * a)
                                   .toString(16).padStart(2, '0')).join('');
}
/// Barva, kterou je třeba zadat, aby se 3D plocha vykreslila jako `hex`.
/// Když na to světlo nestačí, zachová se odstín a sníží jas.
function proSvetlo(hex, f) {
  f = f || faktorSvetla();
  const c = hexNaRgb(hex);
  const raw = [c[0] / Math.max(0.04, f[0]), c[1] / Math.max(0.04, f[1]), c[2] / Math.max(0.04, f[2])];
  const m = Math.max(raw[0], raw[1], raw[2], 255);
  return '#' + raw.map((x) => Math.max(0, Math.min(255, Math.round(x * 255 / m)))
                              .toString(16).padStart(2, '0')).join('');
}
...
/// `svetlo.js` volá po každé změně `mapa.setLight` – barva desky se počítá
/// ze světla, takže se most musí přestavět (jinak zůstane ranní odstín i večer).
window.__mostyPrebarvi = function () {
  try { naplanujMosty3d(600); } catch (e) { /* engine se zrovna staví */ }
  try {   // okna i domy mají barvu ze světla – přepočítat
    if (mapa.getLayer('okolnik-okna-3d')) {
      mapa.setPaintProperty('okolnik-okna-3d', 'fill-extrusion-color',
          vyrazBarvyOken(typeof krokNoci === 'number' && krokNoci >= 2));
    }
    nastavZariOken(typeof krokNoci === 'number' && krokNoci >= 2);   // engine 264
    prebarviDomyNoci();
  } catch (e) { /* styl se zrovna mění */ }
};
...
function prepoctiMosty3d() {
  ...
  for (const f of prvky) {
    ...
    for (const c of cary) {
      ...
      // sebe pak stály DVĚ desky posunuté o pár metrů („mosty stále na nic").
      // Body se proto zaokrouhlí na ~2 m a dvojice konců se seřadí.
      const kr = (q) => q[0].toFixed(4) + ',' + q[1].toFixed(4);
  ...
  const fSv = faktorSvetla();
  // ⛔⛔ engine 247: NESMÍ SE JMENOVAT `B` – uvnitř smyčky mostů je `B` KONCOVÝ
  // BOD úseku, takže `B.zabradli` od enginu 245 vracelo undefined a zábradlí,
  // nosník, opěry i pilíře se kreslily ČERNĚ (černý pruh na snímku uživatele).
  const BARVY3D = {};
  for (const k of Object.keys(MOST_CIL)) BARVY3D[k] = proSvetlo(ztlumNoci(MOST_CIL[k]), fSv);
  const asfaltKes = {};
  const asfaltPro = (cls) => (asfaltKes[cls] || (asfaltKes[cls] =
      proSvetlo(ztlumNoci(MOST_CIL_ASFALT[cls] || MOST_ASFALT_ZAL), fSv)));
  const features = [];
  const ploche = [];
  let podpis = 0, vPohledu = 0;
  const pridej = (ring, b, h, c) => {
    if (!c) { console.warn('[mosty] díl bez barvy'); c = '#A39D94'; }   // engine 247: nikdy černá
  ...
  for (const [klic, m] of mosty) {
    ...
    // ⭐ engine 247: ÚHEL PODLE PRVNÍHO A POSLEDNÍHO BODU NAPOJENÍ (přání
    // uživatele). Most je konstrukce, ne trasa – rovná deska mezi konci sedí
    // líp než sledování polyčáry, která z dlaždic chodí s drobným zlomem
    // (naměřeno 0–1,2 m). Oblouk (odchylka nad 3 m) si polyčáru nechá.
    let odchMax = 0;
    {
      const ax = A[0], ay = A[1];
      for (const q of cara) {
        const px = (q[0] - ax) * kxM, py = (q[1] - ay) * kyM;
        const t = Math.max(0, Math.min(1, (px * abx + py * aby) / ab2));
        odchMax = Math.max(odchMax, Math.hypot(px - abx * t, py - aby * t));
      }
    }
    ...
    // engine 264 („ještě malinko zvedni"): 1,2 → 1,6 m nad nižším koncem,
    ...
    // ⭐ engine 265 („zaoblit začátek a konec směrem dolů"): deska = střed
    // + dva NÁJEZDY na každém konci, které klesají k silnici (⅓ a ⅔ zdvihu).
    // Každý kus má výšku z TERÉNU VE SVÉM TĚŽIŠTI (MapLibre zvedá prvek
    // o terén v těžišti), takže sedí v absolutní výšce. Kusů je pět a leží
    // u náspů, kde DEM nekolísá – schody z engine 251 dělalo 8+ kusů po
    // 6 m nad údolím.
    ...
    if (najezdy) {
      const dil = dR / 2;
      kusy.push({ bd: [cely[0]].concat(podcara(0, dil)), top: mostovka - zdvihM * ex * 0.66 });
      kusy.push({ bd: podcara(dil, dR), top: mostovka - zdvihM * ex * 0.33 });
    ...
    for (const kus of kusy) {
      if (!kus.bd || kus.bd.length < 2) continue;
      ...
      if (zel) {
        pridej(pas(kus.bd, 0.3, 0.72), bk, bk + 0.15, BARVY3D.kolej);
        pridej(pas(kus.bd, 0.3, -0.72), bk, bk + 0.15, BARVY3D.kolej);
      }
      pridej(pas(kus.bd, 0.3, (w - 0.3) / 2), bk, bk + zbr, BARVY3D.zabradli);
      pridej(pas(kus.bd, 0.3, -(w - 0.3) / 2), bk, bk + zbr, BARVY3D.zabradli);
    }
    ...
    for (let u = 0; u < pocetP; u++) {
      ...
      const ring = [[PP[0] + (ux * s2 + nx * s2) / kxM, PP[1] + (uy * s2 + ny * s2) / kyM],
                    ...
                    [PP[0] + (-ux * s2 + nx * s2) / kxM, PP[1] + (-uy * s2 + ny * s2) / kyM]];
      ring.push(ring[0]);
      pridej(ring, 0, +hP.toFixed(1), BARVY3D.pilir);
    }
  }
...
/// ⛔ „stromy jsou spíš bez barvy než do modra": míchání přes JAS sice ubralo
/// barvu, ale modrou skoro nepřidalo (jas oranžového listí je vysoký, takže
/// výsledek vyšel béžový). Odstín měsíce je proto výrazně modřejší a míchá se
/// MÍŇ – část původní barvy zůstane a modrá převáží.
const NOC_MESIC = [0.42, 0.72, 1.42];          // odstín měsíčního světla
const NOC_MIRA = [0, 0.25, 0.5, 0.62];         // den, soumrak, noc, hluboká noc
...
function aplikujNoc() {
  try {
    ...
    // engine 264 („ve tmě je vážně moc tma"): plná noc 0,78 → 0,66, soumrak
    // 0,45 → 0,40; barva do modra – měsíční noc, ne černá díra
    const TMA = [0, 0.20, 0.40, 0.66];
    if (!mapa.getLayer('noc-prekryv')) {
      ...
      mapa.addLayer({
        ...
        paint: { 'fill-color': '#0B1A3A', 'fill-opacity': TMA[krok] },
    ...
    if (mapa.getLayer('mlha-rytina')) {
      ...
      try {   // engine 241: okna v noci svítí a blikají, rostliny jdou do modra
        ...
        // starý; most se přestaví sám za 600 ms, kdy už je krok zapsaný.
        naplanujMosty3d(600);
