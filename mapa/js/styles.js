  ...
  return {
    ...
    layers: [
      ...
      { id: 'zab-cesty', type: 'line', source: 'krajina', 'source-layer': 'cesty',
        ...
        // engine 375: čitelně (dřív 0,3–0,55 s rozmazáním – „cesty jsou nevýrazné“)
        paint: { 'line-color': '#9A845E', 'line-blur': 0.2,
                 'line-opacity': ['interpolate', ['linear'], ['zoom'], 13, 0.6, 15, 0.85],
                 'line-width': sirkaMetry(1.4, 2.0, 13) } },
      { id: 'zab-pesiny', type: 'line', source: 'krajina', 'source-layer': 'cesty',
        minzoom: 13.5, filter: ['==', ['get', 't'], 'pesina'],
        layout: { 'line-cap': 'round', 'line-join': 'round' },
        paint: { 'line-color': '#A08A63', 'line-blur': 0.2,
                 'line-opacity': ['interpolate', ['linear'], ['zoom'], 13.5, 0.55, 15, 0.8],
                 'line-width': sirkaMetry(1.1, 1.3, 13.5) } },
...
function stylHerni(ctx) {
  return {
    ...
    sources: zdroje(ctx, {
      ...
      // (engine 216: stíny domů = canvas zdroj `stiny-domu` + raster `stin-domu`,
      //  obojí zakládá za běhu main.js `zajistiVrstvuStinu` hned za budovy-vypln)
    ...
    layers: [
      ...
      { id: 'reky', type: 'line', source: 'omt', 'source-layer': 'waterway',
        ...
        paint: { 'line-color': PALETA.tyrkys, 'line-blur': 0.4,
                 // interpolace musí být vnější výraz (limit MapLibre)
                 // ⭐ 8. 9. 2026: PLNÁ ŠÍŘKA jako u silnic (přání „řeky nejsou
                 // v plné šíři? tak to udělej"): řeka 5 → 9 m, potok 2 → 3,6 m
                 'line-width': ['interpolate', ['exponential', 2], ['zoom'],
                   10, ['match', ['get', 'class'], 'river', 2.2, 1.0],
                   22, ['match', ['get', 'class'], 'river', 756, 291]] } },
      ...
      // engine 375: lem + šedý povrch jako ostatní silnice (světle hnědá bez lemu na poli zanikala)
      { id: 'silnice-servisni-lem', type: 'line', source: 'omt',
        'source-layer': 'transportation', minzoom: 14.5,
        filter: ['==', ['get', 'class'], 'service'],
        layout: { 'line-cap': 'round', 'line-join': 'round' },
        paint: { 'line-color': '#6E675C', 'line-opacity': 0.7,
                 'line-width': sirkaMetry(2.0, 3.8, 14.5) } },
      { id: 'silnice-servisni', type: 'line', source: 'omt',
        'source-layer': 'transportation', minzoom: 14.5,
        filter: ['==', ['get', 'class'], 'service'],
        layout: { 'line-cap': 'round', 'line-join': 'round' },
        paint: { 'line-color': '#ADA698',
                 'line-width': sirkaMetry(1.2, 3.0, 14.5) } },
      ...
      // ⭐ engine 216: vržené stíny domů kreslí main.js na PLÁTNO (raster
      // vrstva `stin-domu` vložená za běhu hned sem). Engine 215 měl výplňové
      // mnohoúhelníky: kusy hal z dlaždic z17 se ztrácely a překryvy tmavly.
    ...
    layers: [
      ...
      { id: 'silnice', type: 'line', source: 'omt',
        ...
        filter: ['in', ['get', 'class'],
                 ['literal', ['motorway', 'trunk', 'primary',
                              'secondary', 'tertiary']]],
        paint: { 'line-color': '#b3a78e', 'line-opacity': 0.95,          // engine 375: bylo #cfc6b2 (splývalo)
                 'line-width': sirka(9, 0.6, 16, 3.4) } },
      { id: 'ulice', type: 'line', source: 'omt',
        'source-layer': 'transportation', minzoom: 13,
        filter: ['in', ['get', 'class'],
                 ['literal', ['minor', 'service']]],
        paint: { 'line-color': '#bdb29b', 'line-opacity': 0.95,          // engine 375: bylo #d6cdb9 / 0,8
                 'line-width': sirka(13, 0.7, 16, 2.2) } },
      { id: 'cesty', type: 'line', source: 'omt',
        'source-layer': 'transportation', minzoom: 12,
        filter: ['in', ['get', 'class'],
                 ['literal', ['path', 'track']]],
        paint: { 'line-color': '#8f7d5e', 'line-opacity': 0.9,           // engine 375: bylo #b9ae95 / 0,8
                 'line-width': sirka(12, 0.7, 16, 1.8),
