...
const AnimaceNadMapou = (() => {
  ...
  function vycisti(uplne) {
    ...
    if (uplne) { oblacky.length = 0; krouzky.length = 0; lety.length = 0; kudrlinky.length = 0; sedici = []; sediciSig = '';
                 vrtule = []; vrtuleSig = ''; vrtuleNaPlatne = 0; svit.clear(); odlSig = ''; svitSig = ''; vycistiO(); }
  }
  ...
  function neco(st) {
    if (oblacky.length || krouzky.length || lety.length || kudrlinky.length || svitPrechod()) return true;
    if (necinny()) return false;
    const z = mapa.getZoom();
    if (vrtule.length && z >= 11.6) return true;             // engine 357: točící se listy větrníků
    if (odleskyPotrebuji(st) || svitDycha()) return true;    // engine 358–359: odlesky (přepočet po pohybu, dýchání)
  ...
  // a dráty vedení (evidence). Kreslí se na VLASTNÍ plátno s `mix-blend-mode: plus-lighter` = skutečné aditivní
  // míchání (záře se PŘIČTE ke světlu vody/kovu pod ní), měkké kulaté záře BEZ cípů (⛔ 5. 9. „hvězdičky dej pryč“).
  // ⭐⭐ engine 359 (T: „mohly by se pozice záblesků měnit pouze při posunu kamery a na kovech zpomalit? Prostě aby
  // vypadaly více realisticky.“): ŽÁDNÉ NÁHODNÉ JISKŘENÍ. Každé místo na hladině má PEVNOU plošku vlny (sklon podle
  // větru, z hashe místa) a zablýskne se jen tehdy, když ploška zrcadlí slunce přesně do kamery – při stojící kameře
  // svítí stále tatáž místa (jen jemně dýchají), při pohybu kamery se rozsvěcují jiná, jako skutečná třpytivá
  // stezka. Kov = válce: lesk sedí tam, kde je poloviční vektor slunce–kamera kolmý na osu (na trubce, svodidle
  // i drátu se najde přesné místo) a při pohybu kamery PLYNULE KLOUŽE; rozsvícení a zhasnutí pomalé (0,5 s).
  // Stojí-li kamera a nic se nemění, plátno zůstane nakreslené a smyčka spí (šetří baterii). Jen odkryté (místa jsou
  // z evidence), ve dne slunce > 3° bez deště/sněhu/mlhy a oblačnosti < 0,9; v noci modravě od měsíce, jen voda.
  // Jako ve skutečnosti se hladina rozzáří jen při pohledu PROTI slunci (s větrem v širším pásu).
  const ODL_VODA_MAX = 5000, ODL_KOV_MAX = 2500, ODL_SVIT_VODA = 150, ODL_SVIT_KOV = 60;
  ...
  const TAU_VODA = 0.22, TAU_KOV = 0.5;                           // s – náběh a dohasnutí záblesku
  ...
  let svitSig = '', svitMs = 0, svitSvetlo = null;
  let spriteOdl = null;
  ...
  function obnovOdlesky() {
    ...
    if (evP) {
      for (const t of evP.dlazdice) {
        ...
        if (te < w || tw > e || tn < s || ts > n) continue;
        const d = t.d;
        for (let i = 0; i + 7 < d.length && K.length < ODL_KOV_MAX; i += 8) {
          const hK = ODL_TYP_VYSKA[d[i + 7]];
          if (!hK) continue;
          const lonA = lonT(t.x + d[i]), latA = latT(t.y + d[i + 1]), lonB = lonT(t.x + d[i + 3]), latB = latT(t.y + d[i + 4]);
          const lonM = (lonA + lonB) / 2, latM = (latA + latB) / 2;
          if (lonM < w || lonM > e || latM < s || latM > n) continue;
          K.push({ klic: 'k' + t.x + '/' + t.y + '/' + i,
                   ax: (lonA - c.lng) * kx, ay: (latA - c.lat) * ky, az: d[i + 2] * ex + hK,
                   bx: (lonB - c.lng) * kx, by: (latB - c.lat) * ky, bz: d[i + 5] * ex + hK });
        }
      }
    }
    oK = K.length ? K : null;
    ...
    if (evE && evE.draty) {
      ...
      for (let i = 0; i + 6 < D.length; i += 7) {
        ...
        if (t >= 1) {
          ...
          const ix = Math.round(mx * 1e5), iy = Math.round(my * 1e5);
          oD.push({ klic: 'd' + ix + ',' + iy, a: D[i], b: D[i + 1], c: D[i + 2], d: D[i + 3], ha: D[i + 4], hb: D[i + 5], t, L: dl,
                    ax: (D[i] - c.lng) * kx, ay: (D[i + 1] - c.lat) * ky, bx: (D[i + 2] - c.lng) * kx, by: (D[i + 3] - c.lat) * ky,
                    vod: Math.floor(h32(ix, iy, 21) * 64) });
        }
        if (t >= 2) {
          for (const [lon, lat, h] of [[D[i], D[i + 1], D[i + 4]], [D[i + 2], D[i + 3], D[i + 5]]]) {
            const ix = Math.round(lon * 1e5), iy = Math.round(lat * 1e5), k = ix + ',' + iy;
            if (vid.has(k)) continue;
            vid.add(k);
            // tři pevné plošky příhradové konstrukce na různých konzolách
            const plosky = [0, 1, 2].map((f) => {
              const az = 2 * Math.PI * h32(ix, iy, 31 + f), el = (h32(ix, iy, 41 + f) - 0.3) * 1.2;
              return [Math.cos(el) * Math.sin(az), Math.cos(el) * Math.cos(az), Math.sin(el), h32(ix, iy, 51 + f)];
            });
            oS.push({ klic: 's' + k, lon, lat, h, t, X: (lon - c.lng) * kx, Y: (lat - c.lat) * ky, plosky });
          }
        }
      }
    }
  ...
  const polovicni = (S, K, x, y, z, out) => {                     // H = normalize(S + V), V = k bodu → kamera
    let vx = K[0] - x, vy = K[1] - y, vz = K[2] - z;
    const vl = Math.sqrt(vx * vx + vy * vy + vz * vz) || 1;
    let hx = S[0] + vx / vl, hy = S[1] + vy / vl, hz = S[2] + vz / vl;
    const hl = Math.sqrt(hx * hx + hy * hy + hz * hz) || 1;
    out[0] = hx / hl; out[1] = hy / hl; out[2] = hz / hl;
    return out;
  };
  const Htmp = [0, 0, 0], Htmp2 = [0, 0, 0];
  /// které odlesky svítí – po změně kamery, slunce nebo větru (ne každý snímek); výsledek do `svit` (cíl jasu)
  function sigKamery(L) {
    const c = mapa.getCenter();
    return c.lng.toFixed(6) + ',' + c.lat.toFixed(6) + ',' + mapa.getZoom().toFixed(3) + ',' + mapa.getBearing().toFixed(1) + ','
      + mapa.getPitch().toFixed(1) + ',' + L.S.map((v) => v.toFixed(3)).join(',') + ',' + odlSig;
  }
  function prepoctiSvit(L, t) {
    const sig = sigKamery(L);
    if (sig === svitSig || t - svitMs < 120) return;
    if (oV && Math.abs(vitrKmh() - oVitr) > 3) ploskyVln();
    svitSig = sig; svitMs = t;
    const K = kameraOdl();
    if (!K) return;
    ...
    if (L.kov) {
      ...
      if (oK) {
        const kand = [];
        for (const q of oK) {
          const wx = q.bx - q.ax, wy = q.by - q.ay, wl = Math.hypot(wx, wy) || 1;
          const ux = wx / wl, uy = wy / wl;
          polovicni(S, K, q.ax, q.ay, q.az, Htmp); polovicni(S, K, q.bx, q.by, q.bz, Htmp2);
          const hA = Htmp[0] * ux + Htmp[1] * uy, hB = Htmp2[0] * ux + Htmp2[1] * uy;
          let u = -1, I = 0;
          if (hA * hB <= 0 && hA !== hB) { u = hA / (hA - hB); I = 1; }
          else { const m = Math.min(Math.abs(hA), Math.abs(hB)); if (m < 0.1) { u = Math.abs(hA) < Math.abs(hB) ? 0 : 1; I = 1 - m / 0.1; } }
          if (u < 0 || I < 0.15) continue;
          kand.push({ q, u, I });
        }
        if (kand.length > ODL_SVIT_KOV) kand.sort((p, r) => r.I - p.I).length = ODL_SVIT_KOV;
        for (const { q, u, I } of kand) {
          const x = q.ax + (q.bx - q.ax) * u, y = q.ay + (q.by - q.ay) * u, z = q.az + (q.bz - q.az) * u;
          const o = svit.get(q.klic);
          zapal(q.klic, Math.min(1, 0.3 + 0.7 * I), { druh: 'kov', velM: 0.45 + 0.25 * Math.random() });
          const oo = o || svit.get(q.klic);
          oo.lon = oRef.lon + x / kx; oo.lat = oRef.lat + y / ky; oo.alt = z;     // místo lesku klouže s kamerou
        }
      }
      // dráty: místo lesku na rozpětí (H ⟂ vodič), jeden pevný vodič na rozpětí
      for (const q of oD) {
        const V = window.__vedeniVodice && window.__vedeniVodice[q.t];
        if (!V) continue;
        const wx = q.bx - q.ax, wy = q.by - q.ay, wl = Math.hypot(wx, wy) || 1;
        const ux = wx / wl, uy = wy / wl;
        const zOdh = K[2] - 250;                                    // výška drátu jen pro směr (stačí odhad)
        polovicni(S, K, q.ax, q.ay, zOdh, Htmp); polovicni(S, K, q.bx, q.by, zOdh, Htmp2);
        const hA = Htmp[0] * ux + Htmp[1] * uy, hB = Htmp2[0] * ux + Htmp2[1] * uy;
        if (!(hA * hB <= 0 && hA !== hB)) continue;
        const u = Math.max(0.06, Math.min(0.94, hA / (hA - hB)));
        const eA = vyskaPodpery(q.a, q.b), eB = vyskaPodpery(q.c, q.d);
        if (eA === null || eB === null) continue;
        const [hr, lr] = V[q.vod % V.length];
        const h = q.ha + (q.hb - q.ha) * u, sag = 4 * PRUVES_DRATU[q.t] * q.L * u * (1 - u);
        const o = svit.get(q.klic);
        zapal(q.klic, 0.8, { druh: 'drat', velM: 0.55 });
        const oo = o || svit.get(q.klic);
        oo.lon = q.a + (q.c - q.a) * u; oo.lat = q.b + (q.d - q.b) * u; oo.el = eA + (eB - eA) * u;
        oo.bx = lr * h; oo.by = hr * h - sag;
      }
      // příhradové stožáry: pevné plošky na konzolách
      for (const q of oS) {
        const V = window.__vedeniVodice && window.__vedeniVodice[q.t];
        if (!V) continue;
        polovicni(S, K, q.X, q.Y, K[2] - 250, Htmp);
        for (let f = 0; f < 3; f++) {
          const p = q.plosky[f];
          const d = Math.abs(Htmp[0] * p[0] + Htmp[1] * p[1] + Htmp[2] * p[2]);
          const I = Math.pow(d, 60);
          if (I < 0.2) continue;
          const el = vyskaPodpery(q.lon, q.lat);
          if (el === null) continue;
          const [hr, lr] = V[Math.floor(p[3] * V.length)];
          zapal(q.klic + ':' + f, Math.min(1, I), { druh: 'stozar', lon: q.lon, lat: q.lat, el, bx: lr * q.h, by: hr * q.h, velM: 0.6 });
        }
      }
    }
    for (const [k, o] of svit) if (!rozsviceno.has(k)) o.cil = 0;          // zhasnout (plynule)
  }
  ...
  /// náběh nebo dohasnutí ještě běží (doběhne i v nečinnosti)
  function svitPrechod() {
  ...
  function krokOdlesky(t, dt, st) {
    const L = svetloOdlesku(st);
    if (L) svitSvetlo = L;                                      // poslední platné světlo (dohasínání)
  ...
  function kresliOdlesky(t) {
    if (!svit.size || !svitSvetlo) { vycistiO(); return 0; }
    ...
    const sprite = spriteOdl[svitSvetlo.sprite] || spriteOdl.slunce;
    const ts = t / 1000;
    let n = 0;
    for (const o of svit.values()) {
  ...
  function vykresli(t) {
    ...
    kresliOdlesky(t);                                        // stojí-li vše, plátno odlesků zůstane
